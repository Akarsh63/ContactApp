...
router.post('/contacts/:userId',async (req,res)=>{
    ...
    const ct = await contactmodel.find({ contactname: req.body.name, contactnumber: req.body.number, contactemail: req.body.email });
    const filteredContacts = await Promise.all(ct.map(async (element) => {
...
router.get('/contacts/:userId', async (req, res) => {
    try {
      ...
      const savedcontacts = await contactmodel.find({ _id: { $in: user.Contacts } });
  ...
  router.put('/delete', async (req, res) => {
    try {
        ...
        const contactIndex = contacts.findIndex(contact => contact.toString() === key);
        if (contactIndex !== -1) {
            // If the contact exists in the Contacts array, remove it
            contacts.splice(contactIndex, 1);
            const contactt=await contactmodel.findByIdAndDelete(key)
  ...
  router.put('/update/:key', async (req, res) => {
    try {
        ...
        const updatedcontact = await contactmodel.findByIdAndUpdate(req.params.key,{$set:details});
